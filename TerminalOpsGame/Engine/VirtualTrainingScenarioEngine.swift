...
enum TrainingLineID: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .greyhat: return "Greyhat — test, mitigace, retest"
        case .whitehat: return "Whitehat — etický audit a compliance"
        }
    }
    ...
    var theme: String {
        ...
        case .greyhat: return "Propoj ofenzivní test a obranu: nález musí mít mitigaci, retest a měřitelný risk."
        case .whitehat: return "Veď audit legálně: scope, souhlas, důkazy, report pro management a odpovědné zveřejnění."
        }
    }
}
...
enum VirtualTrainingScenarioEngine {
    ...
    static let greyhatChapters: [TrainingChapter] = [
        chapter(.greyhat, 1, "Scope a souhlas", "pravidla testu, riziko, hranice", "Bez jasného scope je i dobrý úmysl problém.", 3, 5),
        chapter(.greyhat, 2, "První nález", "ověření slabiny, dopad, priorita", "Nález není trofej. Je to závazek navrhnout opravu.", 3, 5),
        chapter(.greyhat, 3, "Mitigace", "kontrola, konfigurace, obranný krok", "Skutečná hodnota testu začíná až opravou.", 3, 5),
        chapter(.greyhat, 4, "Retest", "ověření opravy, regresní riziko", "Nezavřeš incident, dokud oprava opravdu drží.", 3, 5),
        chapter(.greyhat, 5, "Risk register", "pravděpodobnost, dopad, vlastník", "Technická slabina musí být srozumitelná i pro rozhodnutí managementu.", 4, 5),
        chapter(.greyhat, 6, "Purple workshop", "red/blue spolupráce", "Nejlepší týmy se nehádají o vinu. Sdílí důkazy.", 4, 5),
        chapter(.greyhat, 7, "Automatizovaný retest", "skripty, evidence, opakovatelnost", "Co nejde opakovat, nejde důvěryhodně měřit.", 4, 5),
        chapter(.greyhat, 8, "Závěrečný risk board", "prioritizace a road map", "Cíl není vyhrát argument. Cíl je snížit riziko.", 5, 5)
    ]

    static let whitehatChapters: [TrainingChapter] = [
        chapter(.whitehat, 1, "Etický kontrakt", "souhlas, NDA, legal basics", "Etický hacking začíná papírem, ne terminálem.", 4, 5),
        chapter(.whitehat, 2, "Audit prostředí", "asset inventory, policy, data flow", "Neaudituješ stroje. Audituješ odpovědnost firmy.", 4, 5),
        chapter(.whitehat, 3, "Privacy a data", "GDPR, tajemství, minimalizace", "Data, která nepotřebuješ, jsou riziko, které si držíš dobrovolně.", 4, 5),
        chapter(.whitehat, 4, "Secure development", "review, dependency risk, secrets", "Bezpečný kód je proces, ne jednorázový scan.", 4, 5),
        chapter(.whitehat, 5, "Responsible disclosure", "časová osa, kontakt, důkazy", "Pravda bez procesu může ublížit lidem, kterým chceš pomoct.", 5, 5),
        chapter(.whitehat, 6, "Management report", "jazyk rizika, náklady, priority", "Dobré doporučení přežije meeting i rozpočet.", 5, 5),
        chapter(.whitehat, 7, "Compliance drill", "kontroly, důkazy, audit trail", "Audit není byrokracie, když chrání důvěru.", 5, 5),
        chapter(.whitehat, 8, "App Store readiness", "privacy, bezpečnost, review", "Finální produkt musí být bezpečný pro hráče i pro platformu.", 5, 5)
    ]

    static var allChapters: [TrainingChapter] { redhatChapters + bluehatChapters + blackhatChapters + greyhatChapters + whitehatChapters }
    ...
    static func missions(for chapter: TrainingChapter) -> [TrainingMission] {
        ...
        case .greyhat:
            return missionTitlesGreyhat.enumerated().map { mission($0.offset + 1, title: $0.element, chapter: chapter) }
        case .whitehat:
            return missionTitlesWhitehat.enumerated().map { mission($0.offset + 1, title: $0.element, chapter: chapter) }
        }
    }
    ...
    private static let missionTitlesGreyhat = ["Vymezení testu", "Bezpečné ověření", "Návrh mitigace", "Retest opravy", "Risk board výstup"]
    private static let missionTitlesWhitehat = ["Mandát auditu", "Evidence bez úniku", "Compliance kontrola", "Report pro vedení", "Odpovědné uzavření"]
    ...
    static func mission(_ number: Int, title: String, chapter: TrainingChapter) -> TrainingMission {
        ...
        case .greyhat: from = "purple-bridge@greyhat.lab"
        case .whitehat: from = "ethics-office@whitehat.audit"
        }
        let mail = TrainingMail(
            id: "mail-\(missionID)",
            from: from,
    ...
    static func storyText(line: TrainingLineID, chapter: TrainingChapter, missionTitle: String) -> String {
        ...
        case .greyhat:
            return "Greyhat mise \(missionTitle.lowercased()) tě nutí přepínat mezi testem, mitigací a retestem. Každý krok musí snížit riziko, ne jen ukázat slabinu."
        case .whitehat:
            return "Whitehat audit \(missionTitle.lowercased()) řeší souhlas, důkazy, soukromí a report tak, aby technická pravda byla použitelná legálně i manažersky."
        }
    }
    ...
    static func learningGoal(line: TrainingLineID, chapter: TrainingChapter, missionTitle: String) -> String {
        ...
        case .greyhat: return "Naučit se purple-team cyklus: test, důkaz, mitigace, retest, risk register a sdílené učení."
        case .whitehat: return "Naučit se etický audit: scope, souhlas, práce s citlivými daty, disclosure a report bez škody."
        }
    }
    ...
    static func commands(line: TrainingLineID, chapter: TrainingChapter, missionID: String, missionIndex: Int) -> [String] {
        ...
        case .greyhat:
            items.insert(contentsOf: ["scope verify \(missionID)", "attack-sim lab-web.corp.local", "mitigation plan \(missionID)", "retest run \(missionID)", "risk register add \(missionID)"], at: 5)
        case .whitehat:
            items.insert(contentsOf: ["audit scope \(missionID)", "privacy review \(missionID)", "evidence seal \(missionID)", "disclosure draft \(missionID)", "compliance map \(missionID)"], at: 5)
        }
        return items
    }
    ...
    static func line(_ raw: String) -> TrainingLineID? {
        ...
        if value.contains("greyhat") || value.contains("grayhat") || value == "grey" || value == "gray" { return .greyhat }
        if value.contains("whitehat") || value == "white" { return .whitehat }
        return nil
    }
}
